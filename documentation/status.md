...
- [x] **Scroll-Driven Entrance & Modal Suppression (`FloatingCartButton.tsx`)**: Wired `window.scrollY > 80` scroll detection with Framer Motion spring enter/exit transitions (`scale: 0.6 -> 1`, `opacity: 0 -> 1`), ensuring the button appears only after scrolling down, automatically hides when the cart drawer is open (`!isCartOpen`) or any product/platter customization popup is open (`!isProductPopupActive`), and remains hidden on admin routes.
...
- [x] **Global Metadata Engine (`src/app/layout.tsx`)**: Title template system (`default` + `%s | Little Karachi Express`), rich SEO description, 14 Karachi-specific keywords, full OpenGraph with Cloudinary CDN-hosted 1200×630 OG banner (`clk_og_banner.jpg`) + logo fallback (`clk_hd_logo.webp`), Twitter `summary_large_image` card, canonical URL, `robots` config with googleBot controls, extended icon set, `manifest` reference, geo tags (`geo.region: PK-SD`, `geo.placename: Karachi`, `geo.position`, `ICBM`), Apple PWA meta, `msapplication-TileColor`.
- [x] **HTML `lang` attribute**: Changed `<html lang="en">` → `<html lang="en-PK">` for regional SEO signal.
- [x] **SEO Helpers Library (`src/lib/seoHelpers.ts`)**: `generateItemMetadata(slug)` and `generatePlatterMetadata(slug)` — server-side functions building per-dish/platter dynamic metadata with price-aware descriptions, Cloudinary dish photo OG images, Twitter cards, keyword arrays, and canonical URLs from live MongoDB data (fallback to Cloudinary CDN banner).
...
- [x] **OG Banner Image (`public/og-banner.jpg` + Cloudinary CDN)**: Generated branded 1200×630 social share image and uploaded to Cloudinary CDN for 100% external scraper reachability.
